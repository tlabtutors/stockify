...
const InventoryItemEditForm = () => {
  ...
  useEffect(() => {
    const fetchItemData = async () => {
      if (!itemId) return;
      try {
        const res = await fetch(`/api/newItem?id=${itemId}`);
        ...
        if (data.ok && data.item) {
          ...
          let dim_length = "",
            ...
            dim_height = "";
          if (item.dimension) {
            const dimParts = item.dimension.split("x");
            if (dimParts.length === 3) {
              [dim_length, dim_width, dim_height] = dimParts;
            }
          }
          setFormData({
            ...
            sku: item.sku || "",
            unit: item.unit || "cm",
            returnable: item.itemType === "Services" ? false : true,
            capitalAsset: item.capitalAsset || false,
            ...
            itemWeightUnit: item.itemWeightUnit || "g",
            sellingPrice: item.sellingPrice || "",
            costPrice: item.costPrice || "",
            salesAccount: item.salesAccount || "",
            trackInventory: item.trackInventory || false,
            purchaseAccount: item.purchaseAccount || "",
            salesItemDescription: item.salesItemDescription || "",
            purchaseItemDescription: item.purchaseItemDescription || "",
            inventoryAccount: item.inventoryAccount || "",
            inventoryValuation: item.inventoryValuation || "",
            preferredVendor: item.preferredVendor || "",
            openingStock: item.openingStock || 0,
            stockRate: item.stockRate || 0,
          ...
          setShowSalesInfo(!!item.sellingPrice || !!item.salesAccount);
          setShowPurchaseInfo(!!item.costPrice || !!item.purchaseAccount);
          setShowTrackingInventory(item.trackInventory || false);
        }
      } catch (error) {
  ...
  //   setItemType(value);
  //   if (value === "Services") {
  //     setFormData((prev) => ({ ...prev, returnable: false }));
  //   } else {
  //     setFormData((prev) => ({ ...prev, returnable: true }));
  //   }
  // };
  ...
  const handleSubmit = async (e) => {
    ...
    if (images.length === 0 && existingImages.length === 0) {
      toast.error("Kindly attach at least one image before updated.");
    ...
    try {
      ...
      // Add existing images (with their metadata)
  ...
  if (isLoading) {
    return (
      <div className="w-full bg-gray-100 mt-30 mb-10 p-5 flex justify-center items-center h-screen">
      ...
      <form onSubmit={handleSubmit} className="p-5">
        <div>
          ...
          <div className="mt-6 col-span-1">
            <button
              type="submit"
              className="bg-blue-400 hover:cursor-pointer hover:bg-blue-500 text-white px-2 py-2 rounded-md font-medium disabled:opacity-50"
